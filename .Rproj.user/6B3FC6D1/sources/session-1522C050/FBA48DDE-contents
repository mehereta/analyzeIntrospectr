...
#' @param data_quality_type whether it is paired or not
...
statistical.test <- function(dataframe,data_quality_type, number_of_timepoints, group_number, magicgroup_number,plot=TRUE/FALSE){
  ...
  #anova for more than 2 timepoints # filter out the na timepoints
  else {
    mod_data = dataframe %>%
      filter(block!='practice')%>%
      filter(group==group_number) %>%
...
#statistical.test(final_processed_file,'NOT paired',3,1,1,TRUE)
