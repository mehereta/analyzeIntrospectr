...
#' @export
plot.by.frequency.timepoint <- function(dataframe,magiccword_group, word_type_list= list(word1)){
  ...
  print(dataframe %>%
          ...
          ggtitle('Percentage of Responses by All Low Frequency Word Across Timepoints - Group 1')+
          ylab('Percent')+
  ...
  print(dataframe %>%
          ...
          filter(itemType=='magic') %>%
          filter(!is.na(itemType)) %>%
          filter(magic_word_group== magiccword_group) %>%
          group_by(timepoint,words,mouse.clicked_name) %>%
          summarise(resp_count = n()) %>%
          mutate(percent_resp = resp_count/sum(resp_count))%>%
          ggplot(aes(x=words, y = percent_resp*100, fill=mouse.clicked_name))+
          geom_bar(stat="identity", width = 0.7)+
          ggtitle(paste0('Percentage of Responses by Group ', magiccword_group,' Low Frequency Word Across Timepoints - Group 1'))+
          ylab('Percent')+
          theme(axis.text.x = element_text(angle=90, vjust=1, hjust=1))+
          coord_flip()+
          facet_wrap(.~timepoint))
  ...
  for (word in word_type_list){
    ...
    print ( final_processed_file %>%
              ...
              ggplot(aes(x=words, y = percent_resp*100, fill=mouse.clicked_name))+
              geom_bar(stat="identity", width = 0.7)+
              ggtitle(paste0('Percentage of Responses by ',word, ' Word Across Timepoints - Group ','1'))+
              ylab('Percent')+
              theme(axis.text.x = element_text(angle=90, vjust=1, hjust=1))+
              coord_flip()+
              facet_wrap(.~timepoint))
...
#' Checking function
#'
#' @description this is a checking function
#'
#' @param dataframe includes information about the participants
#' @param magicgroup_number the group number of the magic word
